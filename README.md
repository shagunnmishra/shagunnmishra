# ☕💻 Code. Coffee. Curiosity. That pretty much sums it up.

# 🎓 B.Tech Student | Passionate Developer

- 💬 Skilled in: `C` • `Python` • `C++` • `JavaScript` • `HTML` • `CSS`
- 🧠 Exploring new tech, solving problems, and learning 
- 🌐 Interested in web development, AI tools, and building cool stuff
- 📫 Reach me: https://www.linkedin.com/in/shagunn06

# 📈 GitHub Stats
! https://github-readme-stats.vercel.app/api?shagunnmishra=shagunnmishra&show_icons=true&hide_border=true
